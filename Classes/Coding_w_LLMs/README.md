...
| **Code Generation** | Generate code for data analysis | Computer code is also a language. As a result, LLMs can learn the syntax rules (e.g. parentheses, indents) and the semantic intent (i.e. what does this code *do*) in order to link instructions in natural language to code patterns. | LLMs can produce code that appears syntactically valid but fails logically or contextually (e.g. undefined variables, missing imports, or incorrect algorithms). They lack real execution feedback and domain-specific context unless explicitly provided. |
| **Translation** | Translate between different coding languages (e.g. between Python and R) | Because LLMs model syntax and semantics across languages, they can identify equivalent constructs between programming languages and reformulate the same algorithm or function in a target language. This relies on learned mappings between token patterns, control flow, and library usage. | Translations can miss subtle differences in data types, library functions, or execution models. The translated code might not run efficiently or correctly without human review and testing. |
| **Concept Explanations** | Explain complicated concepts or technical details | Using its statistical training on large scientific corpora, an LLM can rephrase or simplify specialized language while preserving the logical structure of the concept. It identifies how terms relate within scientific discourse and rewrites them at the requested complexity level. | Explanations may oversimplify or introduce inaccuracies if the prompt is vague or if domain-specific subtleties exceed the model’s training. It lacks true conceptual understanding or experimental grounding. |
| **Stepwise 'Reasoning'** | Comprehend multistep processes and build a coherent workflow | LLMs simulate reasoning by chaining predictions—each token generation step builds on the prior context. When describing workflows, they can reproduce common procedural patterns (e.g. “import → clean → analyze → visualize”) learned from examples in training data. | Their reasoning is synthetic, not causal. They cannot truly plan or evaluate steps, and errors can propagate through the chain. They may omit critical dependencies or produce workflows that are incomplete or circular without user correction. |
